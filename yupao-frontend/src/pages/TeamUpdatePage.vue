...
onMounted(async () => {
  ...
  if (res?.data.code === 0 && res?.data.data) {
    addTeamData.value = res.data.data;
    addTeamData.value.status = addTeamData.value.status.toString();
    const date = addTeamData.value.expireTime?.toString().split('T')[0];
    const originTime = addTeamData.value.expireTime?.toString().split('T')[1];
    const time = `${originTime?.split(':')[0]}:${originTime?.split(':')[1]}`;
    expireDate = date?.split('-');
    expireTime = time?.split(':');
